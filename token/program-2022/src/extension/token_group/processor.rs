...
pub fn process_initialize_group_member(
    ...
    // scope the mint authority check, since the mint is in the same account!
    {
        ...
        if let Ok(member_pointer) = member_mint.get_extension::<GroupMemberPointer>() {
            if Option::<Pubkey>::from(member_pointer.group_address) != Some(*group_info.key) {
                msg!(
                    "The group-member-pointer extension must reference the group provided in \
                    the instruction"
                );
                return Err(TokenError::InvalidExtensionCombination.into());
            }
